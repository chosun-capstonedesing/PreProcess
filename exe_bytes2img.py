...
def extract_bytes_from_text(text):
    """
    PE 헤더가 포함된 .bytes 텍스트 파일에서 메모리 주소를 기준으로
    PE 헤더(0x400000 ~ 0x401000)를 제거한 뒤 바이트 배열을 추출합니다.
    """
    byte_lines = []
    for line in text.strip().split('\n'):
        ...
        if len(line) > 9 and re.match(r'[0-9A-Fa-f]{8}\s', line[:9]):
            ...
            # PE 헤더 영역 (0x400000 ~ 0x401000) 건너뛰기
            if 0x400000 <= addr < 0x401000:
                continue

            # 유효한 영역이면 바이트 데이터만 추출
